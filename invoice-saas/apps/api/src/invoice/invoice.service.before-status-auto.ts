...
import type { CreateInvoiceInput, UpdateInvoiceInput } from "./invoice.schemas";
...
export async function createInvoice(
  ...
  const highestNumeric = existingInvoices.reduce((highest, invoice) => {
...
async function refreshInvoiceStatuses(businessId: string) {
  const now = new Date();
